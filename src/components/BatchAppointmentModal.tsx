...
export const BatchAppointmentModal: React.FC<BatchAppointmentModalProps> = ({ isOpen, onClose, orders, onConfirm }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition-opacity">
      <div className="bg-white rounded-lg shadow-xl w-[900px] max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50">
          <h2 className="text-lg font-bold text-[#1D2129]">批量发起预约</h2>
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="bg-gray-50 text-gray-500 text-xs text-left">
              <tr>
                ...
                <th className="px-3 py-2 border border-gray-200">预计到货时间</th>
                <th className="px-3 py-2 border border-gray-200">大致车型</th>
              </tr>
            </thead>
            <tbody>
              {orders.map(o => (
                <tr key={o.id} className="text-gray-600">
                  ...
                  <td className="px-3 py-2 border border-gray-200">
                    <input type="datetime-local" className="border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:border-[#165DFF] focus:ring-1 focus:ring-[#165DFF] transition-all" />
                  </td>
                  <td className="px-3 py-2 border border-gray-200">
                    <select className="border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:border-[#165DFF] focus:ring-1 focus:ring-[#165DFF] transition-all">
                      <option>4.2m厢式</option>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-end space-x-3">
          ...
          <button onClick={onConfirm} className="px-6 py-2 rounded bg-[#165DFF] text-white text-sm font-medium hover:bg-blue-600 transition-colors shadow-sm">确定</button>
        </div>
      </div>
    </div>
  );
